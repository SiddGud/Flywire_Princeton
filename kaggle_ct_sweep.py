...
if __name__ == '__main__':
    print("\n" + "="*60)
    print(f"  CT-WEIGHTED SA SWEEP  |  Best to beat: {CURRENT_BEST}")
    print("="*60)
    ...
    # SA sweep: 271 configs across all cores
    alphas = [1.2, 1.5, 2, 3, 5, 8, 10, 15, 20, float('inf')]
    tasks  = [(a, s, be, fe, me, N)
              for a in alphas for s in range(30)
    ...
    sa_df.to_csv(f'{OUT_DIR}/ct_sa_{best_n}.csv', index=False)
    ...
    out = f'{OUT_DIR}/submission_CT_{final_n}.csv'
