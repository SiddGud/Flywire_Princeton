...
print("\nStarting Greedy Pruning (True O(1) update)...")
t = time.time()
...
while total_conflicts > 0:
    ...
    if iteration % 1000 == 0:
        elapsed = time.time() - t
        print(f"  iter {iteration:4d}: {len(active):,} nodes remaining, {total_conflicts:,} conflicts ({elapsed:.2f}s)")

print(f"\n✅ CONVERGED! 0 conflicts after {iteration} removals!")
final = filtered.iloc[sorted(active)].copy()
print(f"Final N = {len(final)}")
final.to_csv('submission_NBLAST_ULTRAFAST.csv', index=False)
