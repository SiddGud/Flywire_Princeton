...
for root, dirs, files in os.walk('/kaggle/input'):
    for f in files:
        if 'ct_weighted_triplets' in f:
            DATA2 = root
            break
    if DATA2: break

if DATA2 is None:
    # fallback: try common Kaggle dataset2 paths
...
CT_FILE    = f'{DATA2}/ct_weighted_triplets.csv'
...
if __name__ == '__main__':
    print("\n" + "="*60)
    print(f"  FINE-GRAINED SA SWEEP  |  Best to beat: {CURRENT_BEST}")
    print("="*60)
    ...
    # Directed string edge sets for grow phase
    banc_str = set(zip(banc_df['src'], banc_df['tgt']))
    fafb_str = set(zip(fafb_df['src'], fafb_df['tgt']))
    mcns_str = set(zip(mcns_df['src'], mcns_df['tgt']))

    # Load pre-built CT-weighted triplets
    print(f"\nLoading CT-weighted triplets from {CT_FILE}...")
    triplets = pd.read_csv(CT_FILE, dtype=str)
    print(f"Pool: {len(triplets):,} triplets")

    # Build integer index edges (once, in main)
    ...
    # SA sweep: Fine-grained alphas around the best known configs
    alphas = [4, 6, 7, 9, 11, 12, 13, 14]
    tasks  = [(a, s, be, fe, me, N)
              for a in alphas for s in range(40)]
    ...
    sa_df.to_csv(f'{OUT_DIR}/fine_sa_{best_n}.csv', index=False)

    # Directed Grow-from-Core
    ...
    out = f'{OUT_DIR}/submission_CT_FINE_{final_n}.csv'
    final_df.to_csv(out, index=False)
    print(f"Saved: {out}")

    # Verify directed isomorphism
    print("\nVerifying...")
    Nv=len(final_df)
